...
/** Add an ingredient to a meal: snap a label (default) or search a database. */
export function FoodInput({
  ...
  return (
    <div className="space-y-2">
      ...
      {tab === "scan" ? (
        <FoodScan meal={meal} dateStr={dateStr} onLogged={onLogged} />
      ) : (
        <FoodSearch meal={meal} dateStr={dateStr} onLogged={onLogged} />
      )}
    </div>
  );
}
